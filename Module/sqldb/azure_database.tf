...
resource "azurerm_sql_server" "demo" {
  ...
  administrator_login_password = "$q1@dmiN123"
}
...
resource "azurerm_sql_database" "jdatest" {
  name                             = "${var.project}-jdadb"
  resource_group_name              = "${var.rg_name}"
  location                         = "${var.location}"
  server_name                      = azurerm_sql_server.demo.name
  edition                          = "Standard"
...
#  resource_group_name = "${var.rg_name}"
#  server_name         = azurerm_sql_server.demo.name
#  start_ip_address    = "${var.end_ip_address}"
#  end_ip_address      = "${var.end_ip_address}"
#}
#resource "azurerm_subnet" "demo-database-1" {
#  name                 = "${var.project}-subnet-database-1"
#  resource_group_name  = "${var.rg_name}"
#  virtual_network_name = "${var.vnet_name}"
#  address_prefix       = "${var.address_prefixes_db}"
#}
